# Python ↔ Java 双向 SSH 通道（统一运维与业务链路）

这是生产环境的固定通道，不再作为临时兜底方案。日常运维由 Mac 直接 SSH 到 Python 机，再通过 Python `127.0.0.1:19022` 的反向通道进入 Java 或传包；Java 机不从 Mac 直连。业务流量由 Python `127.0.0.1:19100` 转发到 Java `127.0.0.1:9100`。

## 旧方案：80/443 机间直连（不再作为默认入口）
